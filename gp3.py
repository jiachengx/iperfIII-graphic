...
def vp_start_gui():
    ...
    # Add logging to scrolledtext function and detect break stephenhsu.20191114_1136
...
# ======================================================
    ...
    def collectAllofConfig(self):
        ...
        if mode == 'Server':
            ...
            if gp3_support.che59.get() == 1:
                if self.isNum(self.entry_srvInterval.get()) != True:
            ...
            if gp3_support.che50.get() == 0:
                ...
                if self.isNum(self.entry_srvPort.get()) == True:
                    dict_config['srvPort'] = self.entry_srvPort.get()
                elif self.isNum(self.entry_srvPort.get()) != True:
                    messagebox.showerror("Server Listened Port", "A non-numeric value encountered")
                    return

        elif mode == 'Client':
            dict_config['mode'] = mode
            if self.isValidiP(self.entry_ipaddr.get()) != True:
            ...
            if gp3_support.che49.get() == 0:
                ...
                if self.isNum(self.entry_cListenedPort.get()) != True:
                    messagebox.showerror("Client Listened Port", "A non-numeric value encountered")
                    return
                else:
                    dict_configig['cPort'] = self.entry_cListenedPort.get()
            if gp3_support.che47.get() == 1:
                if self.isNum(self.entry_cInterval.get()) != True:
                    messagebox.showerror("Client Interval", "A non-numeric value encountered")
                    return
                else:
                    dict_configp['cInterval'] = self.entry_cInterval.get()
            if gp3_support.che51.get() == 1:
                if self.isNum(self.entry_testTime.get()) != True:
            ...
            if gp3_support.che53.get() == 1:
                if self.isNum(self.entry_numOfParallelClient.get()) != True:
            ...
            if gp3_support.che63.get() == 1:
                if self.isNum(self.entry_bw.get()) != True:
            ...
            if gp3_support.che68.get() == 1:
                if self.isNum(self.entry_windowSize.get()) != True:
